...
public class GameRegistry {
	...
	/**
	 * Registers a entity selector factory which is used to create predicates
	 * whenever a command containing selectors is executed Any non vanilla arguments
	 * that you expect has to be registered. Otherwise Minecraft will throw an
	 * CommandException on usage.
	 *
	 * If you want to react to a command like "/kill @e[xyz=5]", you would have to
	 * register the argument "xyz" here and check for that argument in the factory.
	 * One factory can listen to any number of arguments as long as they are
	 * registered here.
	 *
	 * For inter mod compatibility you might want to use "modid:xyz" (e.g.
	 * "forge:min_health") as argument.
	 *
	 * For an example usage, see CustomEntitySelectorTest
	 * 
	 * @param arguments
	 *            Expected string arguments in commands
	 */
	...
	/**
	 * Creates a list of entity selectors using the registered factories. Should
	 * probably only be called by Forge
	 */
	...
	/**
	 * This is now private, you should use either ForgeRegistries constants. Or the
	 * registry passed in during the RegistryEvent.Register<T> event.
	 */
	...
	/**
	 * Retrieves the registry associated with this super class type. If the return
	 * is non-null it is HIGHLY recommended that modders cache this value as the
	 * return will never change for a given type in a single run of Minecraft once
	 * set.
	 *
	 * @param registryType
	...
	/**
	 * @deprecated set your item's {@link Item#getItemBurnTime(ItemStack)} or
	 *             subscribe to {@link FurnaceFuelBurnTimeEvent} instead.
	 */
	@Deprecated
	...
	/**
	 * ObjectHolder can be used to automatically populate public static final fields
	 * with entries from the registry. These values can then be referred within mod
	 * code directly.
	 */
	...
	/**
	 * ItemStackHolder can be used to automatically populate public static final
	 * fields with {@link ItemStack} instances, referring a specific item,
	 * potentially configured with NBT. These values can then be used in things like
	 * recipes and other places where ItemStacks might be required.
	 * <p/>
